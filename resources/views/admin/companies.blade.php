<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Admin | Companies</title>
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Manrope:wght@500;600;700;800&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    @vite(['resources/css/recruiter-dashboard.css', 'resources/css/admin-companies.css'])
</head>

<body>
    @php
        $menuItems = [
            [
                'label' => 'Dashboard',
                'icon' => 'fa-chart-line',
                'active' => false,
                'badge' => null,
                'url' => route('admin.dashboard'),
            ],
            [
                'label' => 'Users',
                'icon' => 'fa-users',
                'active' => false,
                'badge' => null,
                'url' => route('admin.users'),
            ],
            [
                'label' => 'Teams',
                'icon' => 'fa-people-group',
                'active' => false,
                'badge' => null,
                'url' => route('admin.teams'),
            ],
            [
                'label' => 'Companies',
                'icon' => 'fa-building',
                'active' => true,
                'badge' => null,
                'url' => route('admin.companies'),
            ],
            [
                'label' => 'Jobs',
                'icon' => 'fa-briefcase',
                'active' => false,
                'badge' => null,
                'url' => route('admin.jobs'),
            ],
            [
                'label' => 'Moderation',
                'icon' => 'fa-shield-halved',
                'active' => false,
                'badge' => null,
                'url' => route('admin.moderation'),
            ],
            [
                'label' => 'Billing',
                'icon' => 'fa-credit-card',
                'active' => false,
                'badge' => null,
                'url' => route('admin.billing'),
            ],
            [
                'label' => 'Reports',
                'icon' => 'fa-flag',
                'active' => false,
                'badge' => null,
                'url' => route('admin.reports'),
            ],
            [
                'label' => 'Settings',
                'icon' => 'fa-gear',
                'active' => false,
                'badge' => null,
                'url' => route('admin.settings'),
            ],
        ];

        $stats = [
            ['label' => 'Total companies', 'value' => '1,284'],
            ['label' => 'Verified', 'value' => '982'],
            ['label' => 'Pending review', 'value' => '86'],
            ['label' => 'Flagged', 'value' => '12'],
        ];

        $companies = [
            [
                'name' => 'TechNova Pvt Ltd',
                'domain' => 'technova.io',
                'industry' => 'SaaS',
                'tier' => 'Enterprise',
                'status' => 'Verified',
                'jobs' => 42,
                'owner' => 'Alex Johnson',
            ],
            [
                'name' => 'BuildStack Labs',
                'domain' => 'buildstacklabs.com',
                'industry' => 'DevTools',
                'tier' => 'Growth',
                'status' => 'Pending',
                'jobs' => 18,
                'owner' => 'Priya Nair',
            ],
            [
                'name' => 'AI Forge Solutions',
                'domain' => 'aiforge.ai',
                'industry' => 'AI/ML',
                'tier' => 'Growth',
                'status' => 'Verified',
                'jobs' => 27,
                'owner' => 'Diego Martinez',
            ],
            [
                'name' => 'Nimbus Retail',
                'domain' => 'nimbusretail.com',
                'industry' => 'E-commerce',
                'tier' => 'SMB',
                'status' => 'Flagged',
                'jobs' => 9,
                'owner' => 'Sara Lee',
            ],
            [
                'name' => 'Urban Mobility Co',
                'domain' => 'urbanmobility.co',
                'industry' => 'Mobility',
                'tier' => 'Growth',
                'status' => 'Pending',
                'jobs' => 6,
                'owner' => 'Kenji Tanaka',
            ],
        ];

        $verificationQueue = [
            ['name' => 'Arcadia Health', 'submitted' => '1h ago', 'docs' => 'GST + CIN', 'risk' => 'Medium'],
            ['name' => 'Helios Cloud', 'submitted' => '3h ago', 'docs' => 'GST', 'risk' => 'Low'],
            ['name' => 'BluePeak Mining', 'submitted' => '6h ago', 'docs' => 'GST + Address', 'risk' => 'High'],
        ];
    @endphp

    <div class="dashboard-shell">
        <div class="dashboard-frame">
            <aside class="sidebar" id="adminSidebar">
                <div class="brand">HiRist Admin</div>

                <nav class="menu" aria-label="Admin sidebar">
                    @foreach ($menuItems as $item)
                        <a href="{{ $item['url'] ?? '#' }}" class="menu-item {{ $item['active'] ? 'active' : '' }}">
                            <span class="menu-icon"><i class="fas {{ $item['icon'] }}"></i></span>
                            <span>{{ $item['label'] }}</span>
                            @if ($item['badge'])
                                <span class="badge">{{ $item['badge'] }}</span>
                            @endif
                        </a>
                    @endforeach
                </nav>

                <div class="logout-wrap">
                    <a href="#" class="menu-item">
                        <span class="menu-icon"><i class="fas fa-right-from-bracket"></i></span>
                        <span>Logout</span>
                    </a>
                </div>
            </aside>
            <div class="sidebar-backdrop" id="adminBackdrop"></div>

            <main class="content admin-companies-content">
                <div class="topbar">
                    <div>
                        <button type="button" class="menu-toggle" id="menuToggle" aria-label="Open sidebar menu"
                            aria-controls="adminSidebar" aria-expanded="false">
                            <span></span>
                            <span></span>
                            <span></span>
                        </button>
                        <h1>Companies</h1>
                        <p>Review company profiles, verification, and publishing status.</p>
                    </div>
                    <div class="top-actions">
                        <button type="button" class="icon-btn" aria-label="Notifications"><i
                                class="fas fa-bell"></i></button>
                        <button type="button" class="icon-btn" aria-label="Support"><i
                                class="fas fa-circle-info"></i></button>
                    </div>
                </div>

                <section class="summary-grid">
                    @foreach ($stats as $stat)
                        <article class="summary-card">
                            <p class="muted">{{ $stat['label'] }}</p>
                            <h3>{{ $stat['value'] }}</h3>
                        </article>
                    @endforeach
                </section>

                <section class="filters-panel">
                    <div class="filter-bar">
                        <div class="searchbox">
                            <i class="fas fa-magnifying-glass"></i>
                            <input type="text" placeholder="Search company or domain" aria-label="Search companies">
                        </div>
                        <select aria-label="Status filter">
                            <option>All statuses</option>
                            <option>Verified</option>
                            <option>Pending</option>
                            <option>Flagged</option>
                        </select>
                        <select aria-label="Tier filter">
                            <option>All tiers</option>
                            <option>Enterprise</option>
                            <option>Growth</option>
                            <option>SMB</option>
                        </select>
                        <select aria-label="Industry filter">
                            <option>All industries</option>
                            <option>SaaS</option>
                            <option>AI/ML</option>
                            <option>Mobility</option>
                            <option>E-commerce</option>
                            <option>Fintech</option>
                        </select>
                        <div class="filter-actions">
                            <button type="button" class="btn-ghost">Reset</button>
                            <button type="button" class="btn-primary">Filter</button>
                        </div>
                        <div class="action-actions">
                            <button type="button" class="btn-ghost">Export</button>
                            <button type="button" class="btn-primary">Add company</button>
                        </div>
                    </div>
                    <div class="filter-chips" aria-label="Quick filters">
                        <span class="filter-chip active"><i class="fas fa-bolt"></i> Pending review</span>
                        <span class="filter-chip"><i class="fas fa-shield"></i> Verified</span>
                        <span class="filter-chip"><i class="fas fa-flag"></i> Flagged</span>
                        <span class="filter-chip"><i class="fas fa-briefcase"></i> Hiring &gt; 20</span>
                    </div>
                </section>

                <section class="panel companies-panel">
                    <header class="panel-header">
                        <div>
                            <h2>Company directory</h2>
                            <p class="muted" style="margin: 4px 0 0;">Domains, industry, status, and hiring volume.</p>
                        </div>
                        <div class="panel-actions">
                            <button type="button" class="btn-ghost">Bulk actions</button>
                            <button type="button" class="btn-primary">Add company</button>
                        </div>
                    </header>

                    <div class="table-wrap">
                        <table>
                            <thead>
                                <tr>
                                    <th>Name</th>
                                    <th>Domain</th>
                                    <th>Industry</th>
                                    <th>Tier</th>
                                    <th>Status</th>
                                    <th>Open jobs</th>
                                    <th>Owner</th>
                                    <th>Actions</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($companies as $company)
                                    <tr>
                                        <td data-label="Name">{{ $company['name'] }}</td>
                                        <td data-label="Domain">{{ $company['domain'] }}</td>
                                        <td data-label="Industry"><span class="tag">{{ $company['industry'] }}</span>
                                        </td>
                                        <td data-label="Tier"><span class="tag">{{ $company['tier'] }}</span></td>
                                        <td data-label="Status">
                                            @php
                                                $pillClass = match ($company['status']) {
                                                    'Verified' => 'pill green',
                                                    'Pending' => 'pill orange',
                                                    default => 'pill red',
                                                };
                                            @endphp
                                            <span class="{{ $pillClass }}">{{ $company['status'] }}</span>
                                        </td>
                                        <td data-label="Open jobs">{{ $company['jobs'] }}</td>
                                        <td data-label="Owner">{{ $company['owner'] }}</td>
                                        <td data-label="Actions">
                                            <div class="row-actions">
                                                <button class="btn-ghost" type="button">Review</button>
                                                <button class="btn-primary" type="button">Manage</button>
                                            </div>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
                    </div>
                </section>

                <section class="panel queue-panel">
                    <header class="panel-header">
                        <div>
                            <h2>Verification queue</h2>
                            <p class="muted" style="margin: 4px 0 0;">Latest submissions awaiting approval.</p>
                        </div>
                        <div class="panel-actions">
                            <button type="button" class="btn-ghost">Assign</button>
                            <button type="button" class="btn-primary">Open queue</button>
                        </div>
                    </header>

                    <div class="queue-list">
                        @foreach ($verificationQueue as $item)
                            <article class="queue-row risk-{{ strtolower($item['risk']) }}">
                                <div>
                                    <h3>{{ $item['name'] }}</h3>
                                    <p class="muted">Docs: {{ $item['docs'] }}</p>
                                </div>
                                <div class="queue-meta">
                                    <span><i class="fas fa-clock"></i> {{ $item['submitted'] }}</span>
                                    <span class="tag">{{ $item['risk'] }} risk</span>
                                </div>
                                <div class="row-actions">
                                    <button class="btn-ghost" type="button">View</button>
                                    <button class="btn-primary" type="button">Approve</button>
                                </div>
                            </article>
                        @endforeach
                    </div>
                </section>
            </main>
        </div>

        <nav class="mobile-bottom-nav" aria-label="Mobile bottom navigation">
            <a href="{{ route('admin.dashboard') }}" class="mobile-nav-item">
                <i class="fas fa-chart-line"></i>
                <span>Dashboard</span>
            </a>
            <a href="{{ route('admin.users') }}" class="mobile-nav-item">
                <i class="fas fa-users"></i>
                <span>Users</span>
            </a>
            <a href="{{ url('/admin/teams') }}" class="mobile-nav-item">
                <i class="fas fa-people-group"></i>
                <span>Teams</span>
            </a>
            <a href="{{ url('/admin/companies') }}" class="mobile-nav-item active">
                <i class="fas fa-building"></i>
                <span>Companies</span>
            </a>
        </nav>
    </div>

    <script>
        (function() {
            const toggle = document.getElementById('menuToggle');
            const sidebar = document.getElementById('adminSidebar');
            const backdrop = document.getElementById('adminBackdrop');

            if (!toggle || !sidebar || !backdrop) return;

            const closeMenu = () => {
                document.body.classList.remove('menu-open');
                toggle.setAttribute('aria-expanded', 'false');
            };

            const openMenu = () => {
                document.body.classList.add('menu-open');
                toggle.setAttribute('aria-expanded', 'true');
            };

            toggle.addEventListener('click', () => {
                if (document.body.classList.contains('menu-open')) {
                    closeMenu();
                } else {
                    openMenu();
                }
            });

            backdrop.addEventListener('click', closeMenu);

            window.addEventListener('resize', () => {
                if (window.innerWidth > 760) {
                    closeMenu();
                }
            });
        })();
    </script>
</body>

</html>
