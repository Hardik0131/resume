<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Admin | Moderation</title>
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Manrope:wght@500;600;700;800&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    @vite(['resources/css/recruiter-dashboard.css', 'resources/css/admin-moderation.css'])
</head>

<body>
    @php
        $menuItems = [
            [
                'label' => 'Dashboard',
                'icon' => 'fa-chart-line',
                'active' => false,
                'badge' => null,
                'url' => route('admin.dashboard'),
            ],
            [
                'label' => 'Users',
                'icon' => 'fa-users',
                'active' => false,
                'badge' => null,
                'url' => route('admin.users'),
            ],
            [
                'label' => 'Teams',
                'icon' => 'fa-people-group',
                'active' => false,
                'badge' => null,
                'url' => route('admin.teams'),
            ],
            [
                'label' => 'Companies',
                'icon' => 'fa-building',
                'active' => false,
                'badge' => null,
                'url' => route('admin.companies'),
            ],
            [
                'label' => 'Jobs',
                'icon' => 'fa-briefcase',
                'active' => false,
                'badge' => null,
                'url' => route('admin.jobs'),
            ],
            [
                'label' => 'Moderation',
                'icon' => 'fa-shield-halved',
                'active' => true,
                'badge' => '8',
                'url' => route('admin.moderation'),
            ],
            [
                'label' => 'Billing',
                'icon' => 'fa-credit-card',
                'active' => false,
                'badge' => null,
                'url' => route('admin.billing'),
            ],
            [
                'label' => 'Reports',
                'icon' => 'fa-flag',
                'active' => false,
                'badge' => null,
                'url' => route('admin.reports'),
            ],
            [
                'label' => 'Settings',
                'icon' => 'fa-gear',
                'active' => false,
                'badge' => null,
                'url' => route('admin.settings'),
            ],
        ];

        $stats = [
            ['label' => 'Open reports', 'value' => '32'],
            ['label' => 'Flagged jobs', 'value' => '18'],
            ['label' => 'Flagged profiles', 'value' => '11'],
            ['label' => 'Automation hits (24h)', 'value' => '126'],
        ];

        $reports = [
            [
                'subject' => 'Suspicious job posting',
                'type' => 'Job',
                'reporter' => 'Anita K.',
                'status' => 'Open',
                'severity' => 'High',
                'age' => '2h',
            ],
            [
                'subject' => 'Spam recruiter outreach',
                'type' => 'Profile',
                'reporter' => 'Rahul M.',
                'status' => 'Investigating',
                'severity' => 'Medium',
                'age' => '5h',
            ],
            [
                'subject' => 'Inappropriate content',
                'type' => 'Job',
                'reporter' => 'Leena P.',
                'status' => 'Open',
                'severity' => 'High',
                'age' => '7h',
            ],
            [
                'subject' => 'Scam profile',
                'type' => 'Profile',
                'reporter' => 'Naveen S.',
                'status' => 'Closed',
                'severity' => 'Low',
                'age' => '1d',
            ],
        ];

        $queues = [
            [
                'title' => 'Payment fraud risk',
                'entity' => 'Job: Finance Manager at Nimbus',
                'submitted' => '32m ago',
                'risk' => 'High',
            ],
            [
                'title' => 'Content policy match',
                'entity' => 'Profile: user_4382',
                'submitted' => '1h ago',
                'risk' => 'Medium',
            ],
            [
                'title' => 'AI similarity flag',
                'entity' => 'Job: Backend Engineer at Helios',
                'submitted' => '3h ago',
                'risk' => 'Low',
            ],
        ];

        $automations = [
            [
                'name' => 'Profanity filter',
                'desc' => 'Blocks postings with banned terms and notifies moderators.',
            ],
            [
                'name' => 'Domain trust scoring',
                'desc' => 'Auto-flag jobs from newly seen company domains.',
            ],
            [
                'name' => 'Duplicate posting check',
                'desc' => 'Detects similar jobs across companies and queues review.',
            ],
            [
                'name' => 'Link safety scan',
                'desc' => 'Sanitizes external links and strips trackers.',
            ],
        ];
    @endphp

    <div class="dashboard-shell">
        <div class="dashboard-frame">
            <aside class="sidebar" id="adminSidebar">
                <div class="brand">HiRist Admin</div>

                <nav class="menu" aria-label="Admin sidebar">
                    @foreach ($menuItems as $item)
                        <a href="{{ $item['url'] ?? '#' }}" class="menu-item {{ $item['active'] ? 'active' : '' }}">
                            <span class="menu-icon"><i class="fas {{ $item['icon'] }}"></i></span>
                            <span>{{ $item['label'] }}</span>
                            @if ($item['badge'])
                                <span class="badge">{{ $item['badge'] }}</span>
                            @endif
                        </a>
                    @endforeach
                </nav>

                <div class="logout-wrap">
                    <a href="#" class="menu-item">
                        <span class="menu-icon"><i class="fas fa-right-from-bracket"></i></span>
                        <span>Logout</span>
                    </a>
                </div>
            </aside>
            <div class="sidebar-backdrop" id="adminBackdrop"></div>

            <main class="content admin-moderation-content">
                <div class="topbar">
                    <div>
                        <button type="button" class="menu-toggle" id="menuToggle" aria-label="Open sidebar menu"
                            aria-controls="adminSidebar" aria-expanded="false">
                            <span></span>
                            <span></span>
                            <span></span>
                        </button>
                        <h1>Moderation</h1>
                        <p>Review reports, triage risks, and oversee safety automations.</p>
                    </div>
                    <div class="top-actions">
                        <button type="button" class="icon-btn" aria-label="Notifications"><i
                                class="fas fa-bell"></i></button>
                        <button type="button" class="icon-btn" aria-label="Support"><i
                                class="fas fa-circle-info"></i></button>
                    </div>
                </div>

                <section class="summary-grid">
                    @foreach ($stats as $stat)
                        <article class="summary-card">
                            <p class="muted">{{ $stat['label'] }}</p>
                            <h3>{{ $stat['value'] }}</h3>
                        </article>
                    @endforeach
                </section>

                <section class="filters-panel">
                    <div class="filter-bar">
                        <div class="searchbox">
                            <i class="fas fa-magnifying-glass"></i>
                            <input type="text" placeholder="Search reports or entities"
                                aria-label="Search moderation">
                        </div>
                        <select aria-label="Type filter">
                            <option>All types</option>
                            <option>Jobs</option>
                            <option>Profiles</option>
                            <option>Messages</option>
                        </select>
                        <select aria-label="Severity filter">
                            <option>All severities</option>
                            <option>High</option>
                            <option>Medium</option>
                            <option>Low</option>
                        </select>
                        <select aria-label="Status filter">
                            <option>All statuses</option>
                            <option>Open</option>
                            <option>Investigating</option>
                            <option>Closed</option>
                        </select>
                        <div class="filter-actions">
                            <button type="button" class="btn-ghost">Reset</button>
                            <button type="button" class="btn-primary">Filter</button>
                        </div>
                        <div class="action-actions">
                            <button type="button" class="btn-ghost">Export</button>
                            <button type="button" class="btn-primary">Assign</button>
                        </div>
                    </div>
                    <div class="filter-chips" aria-label="Quick filters">
                        <span class="filter-chip active"><i class="fas fa-bolt"></i> High severity</span>
                        <span class="filter-chip"><i class="fas fa-flag"></i> New reports</span>
                        <span class="filter-chip"><i class="fas fa-shield"></i> Jobs</span>
                        <span class="filter-chip"><i class="fas fa-user-shield"></i> Profiles</span>
                    </div>
                </section>

                <section class="panel reports-panel">
                    <header class="panel-header">
                        <div>
                            <h2>Reports</h2>
                            <p class="muted" style="margin: 4px 0 0;">Severity, status, and reporter context.</p>
                        </div>
                        <div class="panel-actions">
                            <button type="button" class="btn-ghost">Bulk actions</button>
                            <button type="button" class="btn-primary">New case</button>
                        </div>
                    </header>

                    <div class="table-wrap">
                        <table>
                            <thead>
                                <tr>
                                    <th>Subject</th>
                                    <th>Type</th>
                                    <th>Reporter</th>
                                    <th>Status</th>
                                    <th>Severity</th>
                                    <th>Age</th>
                                    <th>Actions</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($reports as $report)
                                    <tr>
                                        <td data-label="Subject">{{ $report['subject'] }}</td>
                                        <td data-label="Type"><span class="tag">{{ $report['type'] }}</span></td>
                                        <td data-label="Reporter">{{ $report['reporter'] }}</td>
                                        <td data-label="Status">
                                            @php
                                                $statusClass = match ($report['status']) {
                                                    'Open' => 'pill red',
                                                    'Investigating' => 'pill orange',
                                                    default => 'pill green',
                                                };
                                            @endphp
                                            <span class="{{ $statusClass }}">{{ $report['status'] }}</span>
                                        </td>
                                        <td data-label="Severity">
                                            <span
                                                class="severity {{ strtolower($report['severity']) }}">{{ $report['severity'] }}</span>
                                        </td>
                                        <td data-label="Age">{{ $report['age'] }}</td>
                                        <td data-label="Actions">
                                            <div class="row-actions">
                                                <button class="btn-ghost" type="button">Assign</button>
                                                <button class="btn-primary" type="button">Open</button>
                                            </div>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
                    </div>
                </section>

                <section class="panel queue-panel">
                    <header class="panel-header">
                        <div>
                            <h2>Risk queue</h2>
                            <p class="muted" style="margin: 4px 0 0;">Latest automated flags to triage.</p>
                        </div>
                        <div class="panel-actions">
                            <button type="button" class="btn-ghost">Assign</button>
                            <button type="button" class="btn-primary">Open queue</button>
                        </div>
                    </header>

                    <div class="queue-list">
                        @foreach ($queues as $item)
                            <article class="queue-row risk-{{ strtolower($item['risk']) }}">
                                <div>
                                    <h3>{{ $item['title'] }}</h3>
                                    <p class="muted">{{ $item['entity'] }}</p>
                                </div>
                                <div class="queue-meta">
                                    <span><i class="fas fa-clock"></i> {{ $item['submitted'] }}</span>
                                    <span class="tag">{{ $item['risk'] }} risk</span>
                                </div>
                                <div class="row-actions">
                                    <button class="btn-ghost" type="button">View</button>
                                    <button class="btn-primary" type="button">Approve</button>
                                </div>
                            </article>
                        @endforeach
                    </div>
                </section>

                <section class="panel automation-panel">
                    <header class="panel-header">
                        <div>
                            <h2>Safety automations</h2>
                            <p class="muted" style="margin: 4px 0 0;">Rules and detectors currently active.</p>
                        </div>
                        <div class="panel-actions">
                            <button type="button" class="btn-ghost">Run audit</button>
                            <button type="button" class="btn-primary">Add rule</button>
                        </div>
                    </header>

                    <div class="automation-grid">
                        @foreach ($automations as $auto)
                            <article class="automation-card">
                                <h3>{{ $auto['name'] }}</h3>
                                <p>{{ $auto['desc'] }}</p>
                                <div class="row-actions">
                                    <button class="btn-ghost" type="button">Pause</button>
                                    <button class="btn-primary" type="button">Edit</button>
                                </div>
                            </article>
                        @endforeach
                    </div>
                </section>
            </main>
        </div>

        <nav class="mobile-bottom-nav" aria-label="Mobile bottom navigation">
            <a href="{{ route('admin.dashboard') }}" class="mobile-nav-item">
                <i class="fas fa-chart-line"></i>
                <span>Dashboard</span>
            </a>
            <a href="{{ route('admin.users') }}" class="mobile-nav-item">
                <i class="fas fa-users"></i>
                <span>Users</span>
            </a>
            <a href="{{ url('/admin/teams') }}" class="mobile-nav-item">
                <i class="fas fa-people-group"></i>
                <span>Teams</span>
            </a>
            <a href="{{ url('/admin/moderation') }}" class="mobile-nav-item active">
                <i class="fas fa-shield-halved"></i>
                <span>Moderation</span>
            </a>
        </nav>
    </div>

    <script>
        (function() {
            const toggle = document.getElementById('menuToggle');
            const sidebar = document.getElementById('adminSidebar');
            const backdrop = document.getElementById('adminBackdrop');

            if (!toggle || !sidebar || !backdrop) return;

            const closeMenu = () => {
                document.body.classList.remove('menu-open');
                toggle.setAttribute('aria-expanded', 'false');
            };

            const openMenu = () => {
                document.body.classList.add('menu-open');
                toggle.setAttribute('aria-expanded', 'true');
            };

            toggle.addEventListener('click', () => {
                if (document.body.classList.contains('menu-open')) {
                    closeMenu();
                } else {
                    openMenu();
                }
            });

            backdrop.addEventListener('click', closeMenu);

            window.addEventListener('resize', () => {
                if (window.innerWidth > 760) {
                    closeMenu();
                }
            });
        })();
    </script>
</body>

</html>
